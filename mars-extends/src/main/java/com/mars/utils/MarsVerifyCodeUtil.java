...
public class MarsVerifyCodeUtil {
    /**
     * 验证码类型为仅数字,即0~9
     */
    ...
    public static String generateTextCode(int type, int length, String excludeString) {
        ...
        case TYPE_NUM_ONLY:
            while (i < length) {
                ...
                // 排除特殊字符
                if (null == excludeString || excludeString.indexOf(String.valueOf(t)) < 0) {
                    verifyCode.append(t);
                    i++;
                }
            }
    ...
    public static BufferedImage generateImageCode(String textCode, int width, int height, int interLine, boolean randomLocation, Color backColor, Color foreColor, Color lineColor) {
        ...
        for (int i = 0; i < textCode.length(); i++) {
            fy = randomLocation ? (int) ((new Random().nextDouble() * 0.3 + 0.6) * 30) : fy;
            graphics.setColor(null == foreColor ? generateRandomColor() : foreColor);
            // 将验证码字符显示到图象中
            graphics.drawString(textCode.substring(i, i + 1), fx, fy);
            fx += fsize * 0.9;
        }
        graphics.dispose();
